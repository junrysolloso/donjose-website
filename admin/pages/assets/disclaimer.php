     ...
     <?php
       ...
       if($_SERVER['REQUEST_METHOD'] == "POST") {
         if (isset($_POST['submit'])) {
           $desc = $data->iAvoidInject($_POST['texteditDisclaimer']);
           // Check if empty
           if(empty($desc)) {
           ...
           // Try to update
           if($flag !== 0) {
             $r = $data->iUpdateAssets("editDisclaimer", $desc);
             if($r == 1) {
               ...
               <div class="box box-default">
                 ...
                 <div class="box-body">
                   <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
                     <?php
                       $disc = new process();
                       $discTxt = $disc->iShowAssets("dis");
                     ?>
                     <div class="form-group">
                       <textarea class="form-control" name="texteditDisclaimer" id="texteditDisclaimer" style="height: 250px"><?php echo $discTxt; ?></textarea>
                     </div>
     ...
     <script>
      $(function () {
        $(window).load(function () {
          ...
          $(".subBtnAssets").eq(3).addClass("subBtn");
          $("#navAssets").slideDown('slow');
        })
        $('#texteditDisclaimer').wysihtml5();
       })
     </script>
   </body>
 </html>
