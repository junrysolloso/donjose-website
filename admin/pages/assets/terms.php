     ...
     <?php
       ...
       if($_SERVER['REQUEST_METHOD'] == "POST") {
         if(isset($_POST['submit'])) {
           $desc = $data->iAvoidInject($_POST['texteditTerms']);
           // Check if empty
           if(empty($desc)) {
           ...
           // Try to update
           if($flag !== 0) {
             $r = $data->iUpdateAssets("editTerms", $desc);
             if($r == 1) {
               ...
               <div class="box box-default">
                 ...
                 <div class="box-body">
                   <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
                     <?php
                       $terms = new process();
                       $termsTxt = $terms->iShowAssets("terms");
                     ?>
                     <div class="form-group">
                       <textarea class="form-control" name="texteditTerms" id="texteditTerms" style="height: 250px"><?php echo $termsTxt; ?></textarea>
                     </div>
     ...
     <script>
      $(function () {
        $(window).load(function () {
          ...
          $(".subBtnAssets").eq(1).addClass("subBtn");
          $("#navAssets").slideDown('slow');
        })
        $('#texteditTerms').wysihtml5();
       })
     </script>
   </body>
 </html>
