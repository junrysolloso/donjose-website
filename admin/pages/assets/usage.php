     ...
     <?php
       ...
       if($_SERVER['REQUEST_METHOD'] == "POST") {
         if (isset($_POST['submit'])) {
           $desc = $data->iAvoidInject($_POST['editUsage']);
           // Check if empty
           if(empty($desc)) {
           ...
           // Try to update
           if($flag !== 0) {
             $r = $data->iUpdateAssets("editUsage", $desc);
             if($r == 1) {
               ...
               <div class="box box-default">
                 ...
                 <div class="box-body">
                   <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
                     <?php
                     $usage = new process();
                     $usageTxt = $usage->iShowAssets("usage");
                     ?>
                     <div class="form-group">
                       <textarea class="form-control" name="editUsage" id="editUsage" style="height: 250px"><?php echo $usageTxt; ?></textarea>
                     </div>
     ...
     <script>
      $(function () {
        $(window).load(function () {
          ...
          $(".subBtnAssets").eq(2).addClass("subBtn");
